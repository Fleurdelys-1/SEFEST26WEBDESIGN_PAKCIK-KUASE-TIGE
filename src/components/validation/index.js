...
export default function Validation({ initialTab = 'manual', initialQueryValue = '' }) {
  ...
  const [copiedTxA, setCopiedTxA] = useState(false);
  const [copiedTxB, setCopiedTxB] = useState(false);
  const [toast, setToast] = useState({ show: false, message: '' });
  const [isAutoValidating, setIsAutoValidating] = useState(false);
  const [autoValidateStatus, setAutoValidateStatus] = useState('');
  ...
  const handleCopy = async (text, type) => {
    ...
    if (copySuccess) {
      if (type === 'id') {
        ...
        setCopiedTxA(true);
        setTimeout(() => setCopiedTxA(false), 2000);
      } else if (type === 'txB') {
        setCopiedTxB(true);
        setTimeout(() => setCopiedTxB(false), 2000);
      }
      showToast(language === 'id' ? 'Berhasil disalin' : 'Copied successfully');
    } else {
      showToast(language === 'id' ? 'Gagal menyalin teks' : 'Failed to copy text');
    }
  };

  const copyToClipboard = (text) => {
    handleCopy(text, 'hash');
  };
  ...
  const runAutoValidate = async (value) => {
    setIsAutoValidating(true);
    setIsSearching(true);
    setSearchResult(null);

    const statuses = t('validate.autoStatuses');

    let currentStatusIdx = 0;
    setAutoValidateStatus(statuses[0]);
  ...
  useEffect(() => {
    if (typeof window !== 'undefined') {
      ...
      const verifyParam = urlParams.get('verify') || urlParams.get('id');
      
      if (verifyParam) {
        const cleanValue = cleanVerifyValue(verifyParam);
        if (cleanValue) {
          setActiveTab('qr');
          setSearchValue(cleanValue);
          runAutoValidate(cleanValue);
          return;
        }
      }
    }

    if (isInitialLoad.current && initialQueryValue) {
      const cleanValue = cleanVerifyValue(initialQueryValue);
      if (cleanValue) {
        ...
        if (initialTab === 'qr') {
          ...
          performSearch(cleanValue, true, 'manual');
        }
      }
  ...
  const resultGlassStyle = {
    background: 'linear-gradient(135deg, rgba(0, 84, 97, 0.22) 0%, rgba(1, 135, 144, 0.08) 100%)',
    backdropFilter: 'blur(32px) saturate(220%)',
    WebkitBackdropFilter: 'blur(32px) saturate(220%)',
    border: '1px solid rgba(0, 183, 181, 0.35)',
    boxShadow: '0 35px 90px rgba(0, 0, 0, 0.5), inset 0 1px 2px rgba(244, 244, 244, 0.15), 0 0 40px rgba(0, 183, 181, 0.15)',
  };

  const validationCardStyle = {
    background: 'linear-gradient(135deg, rgba(1, 135, 144, 0.18) 0%, rgba(0, 84, 97, 0.08) 100%)',
    border: '1px solid rgba(0, 183, 181, 0.25)',
    boxShadow: 'inset 0 1px 2px rgba(244, 244, 244, 0.12)',
    backdropFilter: 'blur(10px)',
  };

  const badgeThemeStyle = {
    background: 'rgba(1, 135, 144, 0.18)',
    border: '1px solid rgba(0, 183, 181, 0.25)',
  };

  const txRecordStyle = {
    background: 'linear-gradient(135deg, rgba(0, 84, 97, 0.15) 0%, rgba(1, 135, 144, 0.05) 100%)',
    border: '1px solid rgba(0, 183, 181, 0.15)',
    boxShadow: 'inset 0 1px 1px rgba(244, 244, 244, 0.06)',
    backdropFilter: 'blur(10px)',
  };

  const auditBtnStyle = {
    background: 'linear-gradient(135deg, rgba(0, 84, 97, 0.3) 0%, rgba(1, 135, 144, 0.15) 100%)',
    border: '1px solid rgba(0, 183, 181, 0.3)',
    boxShadow: '0 12px 24px rgba(0, 0, 0, 0.3), inset 0 1px 1px rgba(244, 244, 244, 0.15)',
  };

  const downloadBtnStyle = {
    background: 'linear-gradient(135deg, rgba(0, 183, 181, 0.45) 0%, rgba(1, 135, 144, 0.18) 100%)',
    border: '1px solid rgba(244, 244, 244, 0.3)',
    boxShadow: '0 15px 30px -5px rgba(0, 183, 181, 0.35), inset 0 1px 2px rgba(244, 244, 244, 0.2)',
  };

  const errorPanelStyle = {
    background: 'linear-gradient(135deg, rgba(0, 84, 97, 0.4) 0%, rgba(1, 135, 144, 0.15) 100%)',
    backdropFilter: 'blur(32px) saturate(220%)',
    WebkitBackdropFilter: 'blur(32px) saturate(220%)',
    border: '1px solid rgba(1, 135, 144, 0.5)',
    boxShadow: '0 15px 35px rgba(0, 0, 0, 0.55), inset 0 1px 2px rgba(244, 244, 244, 0.15), 0 0 30px rgba(1, 135, 144, 0.2)',
  };

  const tryAgainBtnStyle = {
    background: 'linear-gradient(135deg, rgba(0, 183, 181, 0.5) 0%, rgba(1, 135, 144, 0.2) 100%)',
    border: '1px solid rgba(244, 244, 244, 0.3)',
    boxShadow: '0 10px 25px -5px rgba(0, 183, 181, 0.3), inset 0 1px 2px rgba(244, 244, 244, 0.15)',
  };
        ...
        <AnimatePresence mode="wait">
                    ...
                    {searchResult.found ? (
                      <>
                        ...
                        >
                          ...
                          <div className="absolute inset-0 shadow-[inset_0_0_40px_rgba(0,183,181,0.05)] pointer-events-none rounded-2xl" />
                          <div className="flex items-start justify-between mb-4">
                            <div className="flex items-start gap-3 flex-1">
                              <div className={`flex-shrink-0 p-2 rounded-full ${searchResult.isActive ? 'bg-[#00B7B5]/20' : 'bg-[#018790]/25'}`}>
                                {searchResult.isActive ? (
                                  <CheckCircle2 className="w-6 h-6 text-[#00B7B5]" />
                                ) : (
                                  <AlertCircle className="w-6 h-6 text-[#018790]" />
                                )}
                              </div>
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-2 flex-wrap">
                                  <span className="px-3 py-1 rounded-full text-xs font-bold text-[#F4F4F4]/80 backdrop-blur-md" style={{
                                    ...
                                    {t('validate.details.status')}: {searchResult.certificate.status}
                                  </span>
                                </div>
                                <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-white font-outfit mb-2">
                                  {searchResult.isActive ? t('validate.details.validCertificate') : t('validate.details.revokedCertificate')}
                                </h3>
                           ...
                           <div className="mt-4 space-y-3">
                            <div
                              ...
                              style={validationCardStyle}
                            >
                              <div className="absolute inset-0 bg-gradient-to-tr from-white/5 to-transparent pointer-events-none rounded-lg" />
                              <button
                                onClick={() => handleCopy(searchResult.certificate.id, 'id')}
                                className="group absolute top-3 right-3 p-1.5 hover:bg-white/10 rounded-md transition-colors duration-200 z-30"
                              >
                                <Copy className={`w-4 h-4 ${copiedId ? 'text-[#00B7B5]' : 'text-[#F4F4F4]/40 hover:text-[#F4F4F4]/70'}`} />
                                <span className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/90 backdrop-blur-md border border-white/15 text-[10px] text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-30 font-poppins">
                                  {copiedId ? t('validate.details.copied') : t('validate.details.copyCredentialId')}
                                </span>
                              </button>
                            ...
                            >
                              <div className="absolute inset-0 bg-gradient-to-tr from-white/5 to-transparent pointer-events-none rounded-lg" />
                              <button
                                onClick={() => handleCopy(searchResult.certificate.hash, 'hash')}
                                className="group absolute top-3 right-3 p-1.5 hover:bg-white/10 rounded-md transition-colors duration-200 z-30"
                              >
                                <Copy className={`w-4 h-4 ${copiedHash ? 'text-[#00B7B5]' : 'text-[#F4F4F4]/40 hover:text-[#F4F4F4]/70'}`} />
                                <span className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/90 backdrop-blur-md border border-white/15 text-[10px] text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-30 font-poppins">
                                  {copiedHash ? t('validate.details.copied') : t('validate.details.copyCertificateHash')}
                                </span>
                              </button>
                            ...
                            <div>
                              ...
                              <div className="flex flex-wrap gap-2">
                                {searchResult.certificate.credentials.instructors.map((instructor, idx) => (
                                  <span key={idx} className="px-2 py-1 rounded text-xs text-white/80" style={badgeThemeStyle}>
                                    {instructor}
                                  </span>
                                ))}
                              </div>
                        ...
                        <motion.div
                          ...
                          style={resultGlassStyle}
                        >
                          <div className="absolute inset-0 bg-gradient-to-tr from-white/5 via-transparent to-transparent pointer-events-none rounded-xl" />
                          <div className="flex items-center gap-2 mb-4">
                          ...
                          <div className="space-y-3">
                            ...
                            >
                              ...
                              <span className={`inline-flex items-center gap-2 px-2 py-1 rounded text-xs font-semibold relative z-10 ${searchResult.certificate.blockchainEvidence.smartContractVerified ? 'bg-[#00B7B5]/20 text-[#00B7B5]' : 'bg-[#018790]/25 text-[#018790]'}`}>
                             ...
                             {[
                               ...
                               >
                                 ...
                                 <div className="flex items-center gap-2 mb-2 flex-wrap relative z-10">
                                   ...
                                   <button
                                     onClick={() => handleCopy(item.tx.hash, idx === 0 ? 'txA' : 'txB')}
                                     className="group relative p-1 hover:bg-white/10 rounded transition-colors duration-200 flex-shrink-0"
                                   >
                                     <Copy className={`w-4 h-4 ${(idx === 0 ? copiedTxA : copiedTxB) ? 'text-[#00B7B5]' : 'text-[#F4F4F4]/40'}`} />
                                     <span className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/90 backdrop-blur-md border border-white/15 text-[10px] text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-30 font-poppins">
                                       {(idx === 0 ? copiedTxA : copiedTxB) ? t('validate.details.copied') : (idx === 0 ? t('validate.details.copyIdentityHash') : t('validate.details.copyIntegrityHash'))}
                                     </span>
                                   </button>
                                 </div>
                                <a
                      ...
                      >
                        <div className="absolute inset-0 bg-gradient-to-br from-[#005461]/10 via-transparent to-transparent pointer-events-none" />
                        <AlertCircle className="w-12 h-12 text-[#018790] mx-auto mb-4" />
                      ...
                      <>
                        ...
                        >
                            ...
                            <div className="flex items-start gap-3 flex-1">
                              ...
                              <div className="flex-1 min-w-0">
                                <div className="flex items-center gap-2 mb-2 flex-wrap">
                                  ...
                                  >
                                    {t('validate.details.status')}: {pdfSearchResult.certificate.status}
                                  </span>
                                </div>
                                <h3 className="text-xl sm:text-2xl md:text-3xl font-bold text-white font-outfit mb-2">
                                  {pdfSearchResult.isActive
                                    ? t('validate.details.validCertificate')
                                    : t('validate.details.revokedCertificate')}
                                </h3>
                                <p className="text-sm text-white/70 font-poppins">
                          ...
                          <div className="mt-4 space-y-3">
                            ...
                            >
                              <div className="absolute inset-0 bg-gradient-to-tr from-white/5 to-transparent pointer-events-none rounded-lg" />
                              <button
                                onClick={() => handleCopy(pdfSearchResult.certificate.id, 'id')}
                                className="group absolute top-3 right-3 p-1.5 hover:bg-white/10 rounded-md transition-colors duration-200 z-30"
                              >
                                <Copy className={`w-4 h-4 ${copiedId ? 'text-[#00B7B5]' : 'text-[#F4F4F4]/40 hover:text-[#F4F4F4]/70'}`} />
                                <span className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/90 backdrop-blur-md border border-white/15 text-[10px] text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-30 font-poppins">
                            ...
                            >
                              <div className="absolute inset-0 bg-gradient-to-tr from-white/5 to-transparent pointer-events-none rounded-lg" />
                              <button
                                onClick={() => handleCopy(pdfSearchResult.certificate.hash, 'hash')}
                                className="group absolute top-3 right-3 p-1.5 hover:bg-white/10 rounded-md transition-colors duration-200 z-30"
                              >
                                <Copy className={`w-4 h-4 ${copiedHash ? 'text-[#00B7B5]' : 'text-[#F4F4F4]/40 hover:text-[#F4F4F4]/70'}`} />
                                <span className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/90 backdrop-blur-md border border-white/15 text-[10px] text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-30 font-poppins">
                        ...
                        >
                          ...
                          <div className="space-y-3">
                            ...
                            {[
                              {
                                label: language === 'id' ? 'TX A: Identity (Verifikasi Identitas)' : 'TX A: Identity (Identity Verification)',
                                tx: pdfSearchResult.certificate.blockchainEvidence.txA,
                              },
                              {
                                label: language === 'id' ? 'TX B: Integrity (Verifikasi Integritas Dokumen)' : 'TX B: Integrity (Document Integrity Verification)',
                                tx: pdfSearchResult.certificate.blockchainEvidence.txB,
                              },
                            ].map((item, idx) => (
                              <div
                              ...
                              >
                                ...
                                <div className="flex items-center gap-2 mb-2 flex-wrap relative z-10">
                                  ...
                                  >
                                    <Copy className={`w-4 h-4 ${(idx === 0 ? copiedTxA : copiedTxB) ? 'text-[#00B7B5]' : 'text-[#F4F4F4]/40'}`} />
                                    <span className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/90 backdrop-blur-md border border-white/15 text-[10px] text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-30 font-poppins">
                                      {(idx === 0 ? copiedTxA : copiedTxB) ? (language === 'id' ? 'Tersalin!' : 'Copied!') : (language === 'id' ? `Salin Hash ${idx === 0 ? 'Identity' : 'Integrity'}` : `Copy ${idx === 0 ? 'Identity' : 'Integrity'} Hash`)}
                                    </span>
                                  </button>
                                </div>
                                <a
          ...
          {activeTab === 'qr' && (
            ...
            >
              {isAutoValidating ? (
                <motion.div
                  key="auto-validating-state"
                  initial={{ opacity: 0, y: 20 }}
                  animate={{ opacity: 1, y: 0 }}
                  className="rounded-3xl p-8 text-center space-y-6 relative overflow-hidden flex flex-col items-center justify-center min-h-[300px]"
                  style={resultGlassStyle}
                >
                  <div className="absolute inset-0 bg-gradient-to-br from-[#00B7B5]/5 via-transparent to-white/5 pointer-events-none" />
                  <div className="relative">
                    <div className="absolute inset-0 bg-[#00B7B5]/20 rounded-full blur-xl scale-125" />
                    <motion.div
                      animate={{ rotate: 360 }}
                      transition={{ duration: 1.2, repeat: Infinity, ease: 'linear' }}
                      className="w-16 h-16 rounded-full border-2 border-[#00B7B5]/20 border-t-[#00B7B5] border-r-[#00B7B5]/60 flex items-center justify-center relative z-10"
                    />
                  </div>
                  <div className="space-y-2 relative z-10">
                    <p className="text-xl font-bold text-white font-outfit tracking-wide">
                      {language === 'id' ? 'Validasi Otomatis Sedang Berjalan' : 'Auto-Validation in Progress'}
                    </p>
                    <p className="text-sm text-[#00B7B5] font-mono animate-pulse">
                      {autoValidateStatus}
                    </p>
                  </div>
                </motion.div>
                      ...
                      <div className="space-y-4">
                        <div>
                          ...
                          <div className="mt-4 pt-2">
                            <p className="text-xs text-white/50 mb-2 font-poppins uppercase tracking-wider font-semibold">
                              {language === 'id' ? 'Tautan Validasi Cepat' : 'Quick Validation Link'}
                            </p>
                            <a
                              href="?verify=CERT-C14B116C16F9"
                              className="inline-flex items-center gap-2 px-4 py-2 rounded-xl border border-white/10 hover:border-[#00B7B5]/40 bg-white/5 hover:bg-[#00B7B5]/10 text-white transition-all duration-300 group shadow-md"
                            >
                              <span className="font-mono text-sm text-[#00B7B5] group-hover:text-[#00d6d3] transition-colors">
                                ?verify=CERT-C14B116C16F9
                              </span>
                              <ExternalLink className="w-4 h-4 text-white/40 group-hover:text-[#00B7B5] group-hover:translate-x-0.5 group-hover:-translate-y-0.5 transition-all duration-300" />
                            </a>
                          </div>
                        </div>
                      </div>
                    ...
                    <>
                        ...
                        <div className="mt-4 space-y-3">
                          ...
                          >
                            ...
                            >
                              <Copy className={`w-4 h-4 ${copiedId ? 'text-[#00B7B5]' : 'text-[#F4F4F4]/40 hover:text-[#F4F4F4]/70'}`} />
                              <span className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/90 backdrop-blur-md border border-white/15 text-[10px] text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-30 font-poppins">
                                {copiedId ? (language === 'id' ? 'Tersalin!' : 'Copied!') : (language === 'id' ? 'Salin ID Kredensial' : 'Copy Credential ID')}
                              </span>
                            </button>
                            <p className="text-xs text-[#F4F4F4]/60 uppercase tracking-widest font-semibold mb-1 relative z-10">{t('validate.details.credentialId')}</p>
                            <p className="text-lg font-mono text-[#00B7B5] break-all pr-8 relative z-10">{searchResult.certificate.id}</p>
                          ...
                          >
                            ...
                            >
                              <Copy className={`w-4 h-4 ${copiedHash ? 'text-[#00B7B5]' : 'text-[#F4F4F4]/40 hover:text-[#F4F4F4]/70'}`} />
                              <span className="absolute bottom-full mb-1 left-1/2 -translate-x-1/2 px-2 py-1 bg-black/90 backdrop-blur-md border border-white/15 text-[10px] text-white rounded opacity-0 group-hover:opacity-100 transition-opacity duration-200 pointer-events-none whitespace-nowrap z-30 font-poppins">
                                {copiedHash ? (language === 'id' ? 'Tersalin!' : 'Copied!') : (language === 'id' ? 'Salin Hash Sertifikat' : 'Copy Certificate Hash')}
                              </span>
                            </button>
                            <p className="text-xs text-[#F4F4F4]/60 uppercase tracking-widest font-semibold mb-1 relative z-10">{t('validate.details.certificateHash')}</p>
                            <p className="text-lg font-mono text-[#00B7B5] break-all pr-8 relative z-10">{searchResult.certificate.hash}</p>
                      ...
                      >
                        <div className="rounded-xl p-4 relative overflow-hidden" style={resultGlassStyle}>
                          ...
                          <div className="flex items-center gap-2 mb-3">
                            <div className="w-2 h-2 bg-[#00B7B5] rounded-full" />
                            <p className="text-xs font-bold text-[#F4F4F4]/60 uppercase tracking-widest">{t('validate.details.userInfo')}</p>
                          </div>
                          <div className="space-y-3">
                            <div>
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.fullName')}</p>
                              <p className="text-sm text-white font-poppins">{searchResult.certificate.userInformation.fullName}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.idNumber')}</p>
                              <p className="text-sm font-mono text-white/80">{maskText(searchResult.certificate.userInformation.idNumber)}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.email')}</p>
                              <p className="text-xs font-mono text-white/70 break-all">{maskEmail(searchResult.certificate.userInformation.email)}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.country')}</p>
                              <p className="text-sm text-white/80">{searchResult.certificate.userInformation.country}</p>
                            </div>
                          </div>
                        </div>
                        ...
                        <div className="rounded-xl p-4 relative overflow-hidden" style={resultGlassStyle}>
                          ...
                          <div className="flex items-center gap-2 mb-3">
                            <div className="w-2 h-2 bg-[#00B7B5] rounded-full" />
                            <p className="text-xs font-bold text-[#F4F4F4]/60 uppercase tracking-widest">{t('validate.details.programDetail')}</p>
                          </div>
                          <div className="space-y-3">
                            <div>
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.courseName')}</p>
                              <p className="text-sm text-white/90">{searchResult.certificate.programDetail.courseName}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.eventCode')}</p>
                              <p className="text-sm font-mono text-[#00B7B5]">{searchResult.certificate.programDetail.academicEventCode}</p>
                            </div>
                            <div className="flex gap-2">
                              <div className="flex-1">
                                <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.level')}</p>
                                <p className="text-xs text-white/80">{searchResult.certificate.programDetail.level}</p>
                              </div>
                              <div className="flex-1">
                                <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.language')}</p>
                                <p className="text-xs text-white/80">{searchResult.certificate.programDetail.language}</p>
                              </div>
                              <div className="flex-1">
                                <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.grade')}</p>
                                <p className="text-xs text-white/80">{searchResult.certificate.programDetail.grade}</p>
                              </div>
                            </div>
                          </div>
                        ...
                        <div className="rounded-xl p-4 relative overflow-hidden" style={resultGlassStyle}>
                          ...
                          <div className="flex items-center gap-2 mb-3">
                            <div className="w-2 h-2 bg-[#00B7B5] rounded-full" />
                            <p className="text-xs font-bold text-[#F4F4F4]/60 uppercase tracking-widest">{t('validate.details.certificateDetails')}</p>
                          </div>
                          <div className="space-y-3">
                            <div>
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.institution')}</p>
                              <p className="text-sm text-white/90">{searchResult.certificate.issuingInstitution.name}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">Modality</p>
                              <p className="text-sm text-white/80">{searchResult.certificate.issuingInstitution.modality}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.issuedDate')}</p>
                              <p className="text-sm text-white/80">{searchResult.certificate.dates.issueDate}</p>
                            </div>
                            <div>
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-1">{t('validate.details.studyPeriod')}</p>
                              <p className="text-xs text-[#F4F4F4]/70">{searchResult.certificate.dates.studyPeriodStart} - {searchResult.certificate.dates.studyPeriodEnd}</p>
                            </div>
                          </div>
                        </div>
                      </motion.div>
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 mb-4">
                          <div className="w-2 h-2 bg-[#00B7B5] rounded-full" />
                          <p className="text-xs font-bold text-[#F4F4F4]/60 uppercase tracking-widest">{t('validate.details.credentials')}</p>
                        </div>
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                          <div>
                            <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-2">{t('validate.details.instructors')}</p>
                            <div className="flex flex-wrap gap-2">
                              {searchResult.certificate.credentials.instructors.map((instructor, idx) => (
                                <span key={idx} className="px-2 py-1 rounded text-xs text-white/80" style={badgeThemeStyle}>
                                  {instructor}
                          ...
                          <div>
                            <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-2">{t('validate.details.signatories')}</p>
                            <div className="flex flex-wrap gap-2">
                              {searchResult.certificate.credentials.signatories.map((signatory, idx) => (
                                <span key={idx} className="px-2 py-1 rounded text-xs text-white/80" style={badgeThemeStyle}>
                                  {signatory}
                      ...
                      >
                        ...
                        <div className="flex items-center gap-2 mb-4">
                          <ShieldCheck className="w-5 h-5 text-[#00B7B5]" />
                          <p className="text-xs font-bold text-[#F4F4F4]/60 uppercase tracking-widest"># {t('validate.details.blockchainEvidence')}</p>
                        </div>
                        <div className="space-y-3">
                          <div
                            className="flex items-center justify-between p-3 rounded-lg relative overflow-hidden"
                          ...
                          >
                            <div className="absolute inset-0 bg-gradient-to-tr from-white/5 to-transparent pointer-events-none" />
                            <p className="text-xs text-[#F4F4F4]/60 uppercase tracking-wider font-semibold relative z-10">{t('validate.details.smartContractVerified')}</p>
                            <span className={`inline-flex items-center gap-2 px-2 py-1 rounded text-xs font-semibold relative z-10 ${searchResult.certificate.blockchainEvidence.smartContractVerified ? 'bg-[#00B7B5]/20 text-[#00B7B5]' : 'bg-[#018790]/25 text-[#018790]'}`}>
                              {searchResult.certificate.blockchainEvidence.smartContractVerified ? <CheckCircle2 className="w-3 h-3" /> : <AlertCircle className="w-3 h-3" />}
                              {searchResult.certificate.blockchainEvidence.smartContractVerified ? t('validate.details.verified') : t('validate.details.notVerified')}
                            </span>
                          ...
                          {[
                            { label: language === 'id' ? 'TX A: Identity (Verifikasi Identitas)' : 'TX A: Identity (Identity Verification)', tx: searchResult.certificate.blockchainEvidence.txA },
                            { label: language === 'id' ? 'TX B: Integrity (Verifikasi Integritas Dokumen)' : 'TX B: Integrity (Document Integrity Verification)', tx: searchResult.certificate.blockchainEvidence.txB }
                          ].map((item, idx) => (
                            <div
                              key={idx}
                              className="rounded-lg p-3 relative"
                            ...
                            >
                              <div className="absolute inset-0 bg-gradient-to-tr from-white/5 to-transparent pointer-events-none rounded-lg" />
                              <p className="text-xs text-[#F4F4F4]/50 uppercase tracking-wider font-semibold mb-2 relative z-10">{item.label}</p>
                              <div className="flex items-center gap-2 mb-2 flex-wrap relative z-10">
                                <span className="font-mono text-xs text-[#F4F4F4]/70 break-all flex-1">{item.tx.hash}</span>
                                <button
                                  onClick={() => handleCopy(item.tx.hash, idx === 0 ? 'txA' : 'txB')}
                                  className="group relative p-1 hover:bg-white/10 rounded transition-colors duration-200 flex-shrink-0"
                                >
